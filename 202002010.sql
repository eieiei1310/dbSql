...
FOREIGN KEY 가 생성됨에 따라서 데이터를 삭제할 때 유의점
...
SELECT *
FROM emp_test;
INSERT INTO dept_test VALUES (98, 'ddit1', '대전'); --dept에 데이터를 넣음1
INSERT INTO dept_test VALUES (99, 'ddit2', '대전'); --dept에 데이터를 넣음2
...
옵션에 따라서 부모테이블()삭제시 참조하고 있는 자식 데이터도 같이 삭제된다.
...
부모 테이블의 데이터 삭제시(dept_test)자식테이블에서 참조하는 데이터를 NULL 로 업데이트;
ROLLBACK;
SELECT *
FROM dept_test;
...
ex : 급여 칼럼을 숫자로 관리, 급여가 음수가 들어갈 수 있을까?
     일반적인 경우 급여값은 > 0 
     CHECK 제약을 사용할 경우 급여값이 0보다 큰 값인지 검사 ,
ex2 : EMP 테이블의 job 컬럼에 들어가는 값을 다음 4가지로 제한 가능
...
ex : 컬럼 VARCHAR2(20) ==> VARCHAR2(5)
     기존에 데이터가 존재할 경우 정상적으로 실행이 안 될 확률이 매우 높음
     일반적으로 데이터가 존재하지 않는 상태, 즉 테이블을 생성한 직후에 컬럼의 사이즈, 타입이 잘못된 경우
     컬럼 사이즈, 타입을 변경함.
...
hp 컬럼에는 값을 넣지 않았지만 DEFAULT 설정에 의해 '010' 문자열이 기본값으로 지정된다. 
INSERT INTO emp_test (empno, ename, deptno) VALUES (9999,'brown',99);
...
TABLE 4. 변경시 컬럼 변경
변경하려고 하는 컬럼이 FK제약, PK 제약이 있어도 상관 없음.
...
emp_test 에 hp_n 컬럼이 삭제되었는지를 확인;
DESC emp_test;
...
FOREIGN KEY 테스트;
INSERT INTO emp_test VALUES (9998, 'sally', 34); dept_test 테이블에 존재하지 않는 부서번호이므로 실패;

제약조건 삭제 : PRIMARY KEY, FOREIGN KEY;
ALTER TABLE emp_test DROP CONSTRAINT PK_emp_test;
ALTER TABLE emp_test DROP CONSTRAINT FK_emp_test_dept_test;

제약조건이 없으므로 empno가 중복된 값이 들어갈 수 있고, dept_test 테이블에 존재하지 않는
deptno 값도 들어갈 수 있다;
...
PRIMARY KEY, FOREIGN KEY, 를 해보고
NOT NULL, CHECK, UNIQUE 를 안해봤는데, 그냥 제약조건 이름만 바꿔주면 됨.
...
6. 제약조건을 위배한 들어간 상태에서 제약조건을 활성화;
...
emp_test 테이블에는 empno 컬럼의 값이 9999인 사원이 두 명 존재하기 때문에
...
dept_test 테이블에 존재하지 않는 부서번호 98을 emp_test 에서 사용 중
